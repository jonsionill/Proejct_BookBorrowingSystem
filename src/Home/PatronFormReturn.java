...
//borrowed
  public ArrayList<BorrowedBooks> booklist(){
     ArrayList<BorrowedBooks>booklist = new ArrayList();
       try {
        String query1 = "SELECT * FROM tb_borrowed where library_id = '"+hint.getText()+"'AND status = '"+"Loaned Out"+"'order by loanbook_id desc";
        BookBorrowingSystem.rst= BookBorrowingSystem.stm.executeQuery(query1);
 ...
 public void recordss(){
       ...
       for(int i=0;i<list.size();i++){
           ...
           BorrowedForm.tblmodel.addRow(row);
  ...
  void count(){
       try {
            BookBorrowingSystem.stm = BookBorrowingSystem.con.createStatement();
            BookBorrowingSystem.rst = BookBorrowingSystem.stm.executeQuery("select count(loanbook_id) from tb_borrowed where library_id = '"+hint.getText()+"'AND status = '"+"Loaned Out"+"'order by loanbook_id desc");
            while(BookBorrowingSystem.rst.next()){
      ...
      int pid =0;
        try {
           ...
           staff_id=id;
              ...
              BorrowedForm.searchpatron.setText(pid+"");
              BorrowedForm.patron.setText(pname);
              BorrowedForm.tblmodel.setRowCount(0);
              recordss();
              count();
              dispose();
